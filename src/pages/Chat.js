...
const URL_ANALYSIS = URL + '/get_answer_from_question/';
const URL_CATEGORIES = URL + '/get_categories';
    ...
    >
      ...
      {message.sender === SENDER_STUDY_APP && message.response && (
        <>
          ...
          {message.response.URLs.length > 0 && (
            <Box mt={2}>
          ...
          <MyModal isOpen={isModalOpen === citationDetailsId} onClose={closeModal} title="Citation Details">
            <CitationDetails
              citations={message.response.URLs}
              metadata={message.response.metadata_source_page_to_return}
